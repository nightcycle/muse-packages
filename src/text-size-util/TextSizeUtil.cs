...
namespace TextUtil
{
	...
	public interface ITextSize{
		public static int CHARACTER_HEIGHT_PX = 15;
		public static int LINE_SPACING_PX = 10;
		public static int MIN_SPACER_LEN_PX = 10;
		public static Dictionary<char, int> CHAR_WIDTHS = new Dictionary<char, int>{
			[' '] = 5,
			['!'] = 5,
			['"'] = 8,
			['#'] = 13,
		...
		public static string GetEdgeSpacer(int length, bool isRightSide){
			int spaceWidth = GetStringLength(" ");
		...
		public static string AddStringPadding(string value, int maxValueWidth){

			int spacedMaxWidth = maxValueWidth + MIN_SPACER_LEN_PX*2;

			int netSpacerLength = spacedMaxWidth - GetStringLength(value);
		...
		public static int GetStringLength(string value){
			...
			{
				int charWidth = CHAR_WIDTHS[character];
				length += charWidth;
			}
			return length;
		}

		private static string DebugPrint(){
			StringBuilder output = new StringBuilder($"");

			foreach (KeyValuePair<char, int> item in CHAR_WIDTHS)
			{
